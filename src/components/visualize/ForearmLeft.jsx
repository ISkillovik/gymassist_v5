...
const ForearmLeft = ({ counter, data, newPaddedDate }) => {
  const updateHandleAdd = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.forearmLeft": [...data.userGym.forearmLeft, newPaddedDate, num],
    });
  };

  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.forearmLeft": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.forearmLeft;
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Left forearm"}
      />
      {data.userGym.forearmLeft.length ? (
        <LineChartJS data={data.userGym.forearmLeft} title={"Left forearm"} />
      ) : (
        <p>xuy</p>
      )}
    </div>
