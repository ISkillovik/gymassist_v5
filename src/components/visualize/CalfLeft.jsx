...
const CalfLeft = ({ counter, data, newPaddedDate }) => {
  const updateHandleAdd = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.calfLeft": [...data.userGym.calfLeft, newPaddedDate, num],
    });
  };

  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.calfLeft": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.calfLeft;
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Left calf"}
      />
      {data.userGym.calfLeft.length ? (
        <LineChartJS data={data.userGym.calfLeft} title={"Left calf"} />
      ) : (
        <p>xuy</p>
      )}
    </div>
