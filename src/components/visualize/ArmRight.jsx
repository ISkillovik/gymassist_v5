...
const ArmRight = ({ counter, data, newPaddedDate }) => {
  const updateHandleAdd = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.armRight": [...data.userGym.armRight, newPaddedDate, num],
    });
  };

  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.armRight": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.armRight;
  ...
  const dateConverter = (dateIn) => {
    dateIn.setHours(12);
    let dateIn10days = new Date(dateIn.setDate(dateIn.getDate() + 12));
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Right arm"}
      />
      {data.userGym.armRight.length ? (
        <LineChartJS data={data.userGym.armRight} title={"Right arm"} />
      ) : (
        <p>xuy</p>
      )}
    </div>
