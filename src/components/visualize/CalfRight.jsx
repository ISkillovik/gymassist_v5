...
const CalfRight = ({ counter, data, newPaddedDate }) => {
  ...
  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.calfRight": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.calfRight;
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Right calf"}
      />
      {data.userGym.calfRight.length ? (
        <LineChartJS data={data.userGym.calfRight} title={"Right calf"} />
      ) : (
        <p>xuy</p>
      )}
    </div>
