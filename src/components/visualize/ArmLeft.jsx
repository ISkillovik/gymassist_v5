...
const ArmLeft = ({ counter, data, newPaddedDate }) => {
  const updateHandleAdd = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.armLeft": [...data.userGym.armLeft, newPaddedDate, num],
    });
  };

  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.armLeft": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.armLeft;
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Left arm"}
      />
      {data.userGym.armLeft.length ? (
        <>
          <LineChartJS data={data.userGym.armLeft} title={"Left arm"} />
        </>
      ) : (
        <p>xuy</p>
      )}
