...
const BodyWeight = ({ counter, data, newPaddedDate }) => {
  const updateHandleAdd = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.bodyWeight": [...data.userGym.bodyWeight, newPaddedDate, num],
    });
  };

  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.bodyWeight": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.bodyWeight;
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Weight"}
      />
      {data.userGym.bodyWeight.length ? (
        <LineChartJS data={data.userGym.bodyWeight} title={"Weight"} />
      ) : (
        <p>xuy</p>
      )}
    </div>
