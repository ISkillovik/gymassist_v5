...
const Waistline = ({ counter, data, newPaddedDate }) => {
  const updateHandleAdd = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.waistline": [...data.userGym.waistline, newPaddedDate, num],
    });
  };

  const inputDateValDoc = async (num) => {
    await updateDoc(doc(db, "users", counter.uid), {
      "userGym.lastData.waistline": dateConverter(new Date()),
    });
  };

  const inputDateVal = data.userGym.lastData.waistline;
  ...
  return (
    <div>
      <ParamInput
        ...
        title={"Waistline"}
      />
      {data.userGym.waistline.length ? (
        <LineChartJS data={data.userGym.waistline} title={"Waistline"} />
      ) : (
        <p>xuy</p>
      )}
    </div>
